...
const a_user_calls_editMyProfile = async (user, input) => {
  ...
  const editMyProfile = `mutation editMyProfile {
    editMyProfile(input: {
      name: "${name}"
      bio: "${bio}"
    }) {
  ...
  const data = await GraphQL(process.env.API_URL, editMyProfile, {}, user.accessToken)
...
const we_invoke_getImageUploadUrl = async (username, extension, contentType) => {
  const handler = require('../../functions/get-signed-url').handler
